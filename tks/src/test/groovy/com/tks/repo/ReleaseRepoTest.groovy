...
public class ReleaseRepoTest extends BaseRepoTest<ReleaseRepo,ReleaseRule> {
    ...
    def "P: Find All Release"() {
        ...
        expect:
        List<Release> releases = rule.verifyFindAll(repo, repo.count());
        for (Release s : releases) {
            System.out.println("result is:" + s)
        }
    }

    @Unroll("Find All Release by Join,expecting #result")
    def "P: Find All Release by Join"() {
        setup:
        rule.createAndVerify(repo);
