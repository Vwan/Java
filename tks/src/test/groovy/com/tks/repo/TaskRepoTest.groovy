...
public class TaskRepoTest extends BaseRepoTest<TaskRepo,TaskRule> {
    ...
    @Unroll("Find all tasks by taskStatus")
    def "P:Find All tasks by status"() {
        setup:
        rule.createAndVerify(repo);
        when:
        List<Task> tasks = repo.findByStatus(repo.getEntity().getTaskStatus());
        then:
        assertNotNull(tasks)
        for (Task task : tasks) {
            System.out.println("result is:" + task)
        ...
        @Unroll("findByStatusName - Find all tasks by taskStatus name")
        def "P:Find All tasks by status name"(){
            setup:
            rule.createAndVerify(repo);
            repo.getEntity().setSummary("find all tasks by staus name");
            repo.getEntity().setTaskStatus(repo.getEntity().getTaskStatus());
            def repo2 = new TaskRepo()
            repo2.create(new Task("find all tasks by taskStatus name","",repo.getEntity().getRelease(),repo.getEntity().getTaskStatus()));

            when:
            List<Task> tasks = repo.findByStatusName(repo.getEntity().getTaskStatus().getName());
            then:
            assertNotNull(tasks)
            tasks.size()==2
            for (Task task : tasks) {
    ...
    def "P:Find All tasks by release name"(){
        ...
        def repo2 = new TaskRepo()
        repo2.create(new Task("find all tasks by release name1","",repo.getEntity().getRelease(),repo.getEntity().getTaskStatus()));
    ...
    def "P: Update Existing Task"() {
        ...
        TaskStatus status = new TaskStatus("test","test");
        new TaskStatusRepo().create(status);
